...
pub(crate) fn convert(
    ...
    let hinting_ctx = if state.opt.hinting.enabled {
        Some(crate::text::flatten::HintingContext { enabled: true })
    } else {
        None
    };
...
fn convert_font(node: SvgNode, state: &converter::State) -> Font {
    ...
    let hinting = parse_hinting_settings(node);
...
/// Parses all `resvg:hinting-*` CSS properties (also available as `-resvg-hinting-*` in CSS style).
fn parse_hinting_settings(node: SvgNode) -> HintingSettings {
    let target = node
        .find_attribute::<&str>(AId::ResvgHintingTarget)
        .and_then(|s| s.parse().ok())
        .unwrap_or_default();

    let mode = node
        .find_attribute::<&str>(AId::ResvgHintingMode)
        .and_then(|s| s.parse().ok())
        .unwrap_or_default();

    let engine = node
        .find_attribute::<&str>(AId::ResvgHintingEngine)
        .and_then(|s| s.parse().ok())
        .unwrap_or_default();

    let symmetric_rendering = node
        .find_attribute::<&str>(AId::ResvgHintingSymmetric)
        .map(|s| s == "true")
        .unwrap_or(true); // default is true

    let preserve_linear_metrics = node
        .find_attribute::<&str>(AId::ResvgHintingPreserveLinearMetrics)
        .map(|s| s == "true")
        .unwrap_or(false); // default is false

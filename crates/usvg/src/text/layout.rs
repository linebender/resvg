...
impl PositionedGlyph {
    ...
    pub fn cbdt_transform(&self, x: f32, y: f32, pixels_per_em: f32) -> Transform {
        // Bitmap glyphs are in pixel units, not font units.
        // self.transform() includes scale = font_size / units_per_em (for outline glyphs).
        // We need to counteract that and apply the correct bitmap scaling.
        // Total scale should be: font_size / pixels_per_em
        // So we need: (font_size / units_per_em) * scale = font_size / pixels_per_em
        // Therefore: scale = units_per_em / pixels_per_em
        //
        // When font_size == pixels_per_em, total scale = 1.0 (pixel-perfect rendering).
...
fn compute_effective_variations(
    ...
    if font_optical_sizing == crate::FontOpticalSizing::Auto {
        let has_explicit_opsz = variations.iter().any(|v| v.tag == *b"opsz");
        if !has_explicit_opsz && fontdb.has_opsz_axis(font_id) {

...
pub(crate) fn convert(tree: &Tree, opt: &WriteOptions) -> String {
    ...
    let mut xml = XmlWriter::new(&mut buf, xml_opt);

    xml.start_svg_element(EId::Svg).unwrap();
    xml.write_svg_attribute(AId::Width, &tree.size.width()).unwrap();
    xml.write_svg_attribute(AId::Height, &tree.size.height()).unwrap();
    xml.write_attribute("xmlns", "http://www.w3.org/2000/svg").unwrap();
    if has_xlink(&tree.root) {
        xml.write_attribute("xmlns:xlink", "http://www.w3.org/1999/xlink").unwrap();
    }

    if tree.has_defs_nodes() || tree.has_text_nodes() {
        write_defs(tree, opt, &mut xml).unwrap();
    }

    write_elements(&tree.root, false, opt, &mut xml).unwrap();

    xml.end_document().unwrap();
    String::from_utf8(buf).unwrap()
}

fn write_filters<W: Write>(tree: &Tree, opt: &WriteOptions, xml: &mut XmlWriter<'_, W>) -> io::Result<()> {
...
fn write_defs<W: Write>(tree: &Tree, opt: &WriteOptions, xml: &mut XmlWriter<'_, W>) -> io::Result<()> {
...
fn write_text_path_paths<W: Write>(parent: &Group, opt: &WriteOptions, xml: &mut XmlWriter<'_, W>) -> io::Result<()> {
    for node in &parent.children {
        ...
        node.subroots(|subroot| write_text_path_paths(subroot, opt, xml).unwrap());
    }

    Ok(())
}

fn write_elements<W: Write>(parent: &Group, is_clip_path: bool, opt: &WriteOptions, xml: &mut XmlWriter<'_, W>) -> io::Result<()> {
...
fn write_element<W: Write>(node: &Node, is_clip_path: bool, opt: &WriteOptions, xml: &mut XmlWriter<'_, W>) -> io::Result<()> {
    match node {
        Node::Path(ref p) => {
...
fn write_group_element<W: Write>(g: &Group, is_clip_path: bool, opt: &WriteOptions, xml: &mut XmlWriter<'_, W>) -> io::Result<()> {
    if is_clip_path {
...
trait XmlWriterExt {
    fn start_svg_element(&mut self, id: EId) -> io::Result<()>;
    fn write_svg_attribute<V: Display + ?Sized>(&mut self, id: AId, value: &V) -> io::Result<()>;
    fn write_id_attribute(&mut self, id: &str, opt: &WriteOptions) -> io::Result<()>;
    fn write_color(&mut self, id: AId, color: Color) -> io::Result<()>;
    fn write_units(&mut self, id: AId, units: Units, def: Units) -> io::Result<()>;
    fn write_transform(&mut self, id: AId, units: Transform, opt: &WriteOptions) -> io::Result<()>;
    fn write_visibility(&mut self, value: bool) -> io::Result<()>;
    fn write_func_iri(&mut self, aid: AId, id: &str, opt: &WriteOptions) -> io::Result<()>;
    fn write_rect_attrs(&mut self, r: NonZeroRect) -> io::Result<()>;
    fn write_numbers(&mut self, aid: AId, list: &[f32]) -> io::Result<()>;
    fn write_image_data(&mut self, kind: &ImageKind) -> io::Result<()>;
    fn write_filter_input(&mut self, id: AId, input: &filter::Input) -> io::Result<()>;
    fn write_filter_primitive_attrs(&mut self, parent_rect: NonZeroRect, fe: &filter::Primitive) -> io::Result<()>;
    fn write_filter_transfer_function(&mut self, eid: EId, fe: &filter::TransferFunction) -> io::Result<()>;
}

impl<W: Write> XmlWriterExt for XmlWriter<'_, W> {
    #[inline(never)]
    fn start_svg_element(&mut self, id: EId) -> io::Result<()> {
    ...
    #[inline(never)]
    fn write_color(&mut self, id: AId, c: Color) -> io::Result<()> {
        ...
        self.write_attribute_raw(id.to_str(), |buf| {
            buf.write_all(&[b'#', r1, r2, g1, g2, b1, b2])?;
            Ok(())
        })
    }
    ...
    fn write_transform(&mut self, id: AId, ts: Transform, opt: &WriteOptions) -> io::Result<()> {
        if !ts.is_default() {
            self.write_attribute_raw(id.to_str(), |buf| {
                buf.write_all(b"matrix(")?;
    ...
    fn write_numbers(&mut self, aid: AId, list: &[f32]) -> io::Result<()> {
        self.write_attribute_raw(aid.to_str(), |buf| {
            ...
            if !list.is_empty() {
                // This is not supported by `impl Write`.
                // buf.pop();
            }
    ...
    fn write_image_data(&mut self, kind: &ImageKind) -> io::Result<()> {
        ...
        self.write_attribute_raw("xlink:href", |buf| {
            buf.write_all(b"data:image/")?;
            buf.write_all(mime.as_bytes())?;
            buf.write_all(b";base64, ")?;
...
fn write_base_grad<W: Write>(g: &BaseGradient, opt: &WriteOptions, xml: &mut XmlWriter<'_, W>) -> io::Result<()> {
        ...
        for seg in path.data.segments() {
            match seg {
                PathSegment::MoveTo(p) => {
                    s.push_str("M ");
                    write_num_str(p.x, &mut s, opt.coordinates_precision);
                    s.push(' ');
                    write_num_str(p.y, &mut s, opt.coordinates_precision);
                    s.push(' ');
                }
                ...
                PathSegment::QuadTo(p1, p) => {
                    s.push_str("Q ");
                    write_num_str(p1.x, &mut s, opt.coordinates_precision);
                    s.push(' ');
                    write_num_str(p1.y, &mut s, opt.coordinates_precision);
                    s.push(' ');
                    write_num_str(p.x, &mut s, opt.coordinates_precision);
                    s.push(' ');
                    write_num_str(p.y, &mut s, opt.coordinates_precision);
                ...
                PathSegment::CubicTo(p1, p2, p) => {
                    ...
                    write_num_str(p1.y, &mut s, opt.coordinates_precision);
                    s.push(' ');
                    write_num_str(p2.x, &mut s, opt.coordinates_precision);
                    s.push(' ');
                    write_num_str(p2.y, &mut s, opt.coordinates_precision);
                    s.push(' ');
                    write_num_str(p.x, &mut s, opt.coordinates_precision);
                    s.push(' ');
                    write_num_str(p.y, &mut s, opt.coordinates_precision);
...
fn write_fill<W: Write>(fill: &Option<Fill>, is_clip_path: bool, opt: &WriteOptions, xml: &mut XmlWriter<'_, W>) -> io::Result<()> {
...
fn write_stroke<W: Write>(stroke: &Option<Stroke>, opt: &WriteOptions, xml: &mut XmlWriter<'_, W>) -> io::Result<()> {
...
fn write_paint<W: Write>(aid: AId, paint: &Paint, opt: &WriteOptions, xml: &mut XmlWriter<'_, W>) -> io::Result<()> {
    match paint {
...
fn write_light_source<W: Write>(light: &filter::LightSource, xml: &mut XmlWriter<'_, W>) -> io::Result<()> {
    match light {
...
fn write_num(num: f32, buf: &mut impl Write, precision: u8) -> io::Result<()> {
    if num.fract().approx_zero_ulps(4) {
        write!(buf, "{}", num as i32)?;
    } else {
...
fn write_num_str(num: f32, buf: &mut String, precision: u8) {
